  ...
  async getCoordinatesFromAddress(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to get coordinates from Geoapify',
      );
    }
  }
    ...
    try {
      ...
      for (const feature of features) {
        ...
        if (cat.includes('education'))
          minDistances['scuole'] = Math.min(
            minDistances['scuole'] ?? Infinity,
            distance,
          );

        if (cat.includes('leisure'))
          minDistances['parchi'] = Math.min(
            minDistances['parchi'] ?? Infinity,
            distance,
          );

        if (cat.includes('public_transport'))
          minDistances['trasporti'] = Math.min(
            minDistances['trasporti'] ?? Infinity,
            distance,
          );
      }
